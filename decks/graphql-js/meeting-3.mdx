...
# [Digital Products Boosters](https://boosters.intrepid.digital.accenture.com/)
...
Imagine you are creating a music app where you can play songs that you have bought in other music apps. Product wants to build an album details page that is called the “Songs” page. However, the third-party API with which you’re integrating calls these items “Tracks”. What should the GraphQL type name be for this object? Song or Track?
...
Data loader is the recommended JavaScript library to avoid N + 1 database queries in GraphQL. How does a loader know that it’s done loading objects and is ready to make a single batched request? (Feel free to google “github dataloader”)
...
JavasScript is synchronous and single-thread, but it’s event loop enables it to run additional code after the initial code execution (i.e. setTimeout callback executes after time has passed). Data loader uses Node’s process.nextTick to do a single request to your database after the first event loop iteration where all the loading was done.
